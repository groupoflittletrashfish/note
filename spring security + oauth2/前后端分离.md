    ...
    public class MyExtendOAuth2Exception extends OAuth2Exception {
        ...
        */
        ...
        public MyExtendOAuth2Exception(String msg, String oAuth2ErrorCode, Throwable t) {
            ...
            this.myExtendMessage = oAuth2ErrorMessage != null ? oAuth2ErrorMessage : oAuth2ErrorCode;
        }
...
* 最后需要在AuthorizationServerConfig，也就是认证服务的配置类中添加如下
    ...
    ~~~
* 这样就完成了，完成后就可以生效了，但是如果是/oauth/token接口去获取token的时候，如果是client_id这个参数错误，还是会抛出如下,造成这个原因是因为该错误是Security抛出的，而不是Oauth，想解决这个问题，继续
    ~~~json
    {
        "error": "unauthorized",
...
* 自定义一个身份验证失败的处理类
    ...
    public class EntryPointUnauthorizedHandler implements AuthenticationEntryPoint {
        ...
        public void commence(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, AuthenticationException e) throws IOException, ServletException {
            ...
            String str = JSONObject.toJSONString(R.error(-1, "客户端验证失败"));
    ...
    ~~~
* 并且在Security的总配置中配置，即WebSecurityConfigurerAdapter实现类中，代码片段如下
    ~~~java
        @Override
        protected void configure(HttpSecurity http) throws Exception {
            http.csrf().disable().authorizeRequests()
...
## Oauth2的自定义认证模式（手机短信认证）
* Oauth2默认的模式只有4种，但是可以自定义扩展，参考如下：
* > https://blog.csdn.net/qq_43692950/article/details/122569216
* 以下只介绍最精简的方式
* 由于是认证功能，所以需要写在认证服务器中
* 新建一个认证类型
    ~~~java
    import lombok.extern.slf4j.Slf4j;
    import org.apache.commons.lang3.StringUtils;
    ...
    /**
    * @author ：liwuming
    * @date ：Created in 2022/6/16 11:19
    * @description ：
    * @modified By：
    * @version:
    */
    ...
    public class SmsCodeTokenGranter extends AbstractTokenGranter {
        ...
        /**
        * 自定义的认证类型，也就是grant_type=sms_code
        */
        ...
        protected OAuth2Authentication getOAuth2Authentication(ClientDetails client,TokenRequest tokenRequest) {
            ...
            // 客户端提交的验证码，由于是DEMO,所以此处就写死了
            String smsCode = parameters.get("sms_code");
            if (!smsCode.equals("9876")) {
                throw new AccessDeniedException("code err!");
            }
            //获取用户的身份信息，也就是先通过手机查询到用户名，然后再通过原来的 UserDetailsService的来获取。由于是Demo直接就写死了
            //当然也可以不通过用户名去获取，直接通过手机号去获取用户的信息也可以，这个视具体情况而定
    ...
    ~~~
* 在认证服务的配置中，也就是AuthorizationServerConfig进行配置
    ~~~java
    @Configuration
    @EnableAuthorizationServer
    @RequiredArgsConstructor
    ...
    ~~~
* 由于使用的是JDBC来控制client的信息，在对应的测试客户端的授权类型中添加sms_code类型
* ![](./pic/自定义授权类型-数据库.jpg)
* 使用Postman进行调试，访问 /oauth/token
* ![](./pic/自定义授权类型-身份验证.jpg)
* ![](./pic/自定义授权类型-参数.jpg)


## 密码模式下的账号锁定
* 在通常的场景下，多次输入错误的密码将会进行密码的锁定
* 需要引入ratelimitj依赖，ratelimitj还有基于内存/redis等场景的依赖包，主要用于限流等场景，可以了解下
    ~~~xml
            <dependency>
...
* 自定义一个登录失败的事件
    ...
    public class UserLoginFailedEvent extends ApplicationEvent {
        ...
        public UserLoginFailedEvent(String username) {
            super(username);
        }
    }
    ~~~
* 自定义监听器,代码都是一些demo，也比较好理解
    ~~~java
    @Slf4j
    @Component
    public class UserLoginFailedListener implements ApplicationListener<UserLoginFailedEvent> {
        ...
        public void onApplicationEvent(UserLoginFailedEvent event) {
            ...
            if (reachLimit) {
                throw new LockedException(username + " 账户被锁定");
            }
        }
    }
    ~~~
* 正常来讲，密码模式都是通过 UserDetailsService（自定义的登录逻辑）来验证的
    ~~~java
    @Service
    @RequiredArgsConstructor(onConstructor = @__(@Autowired))
    public class UserDetailsServiceImpl implements UserDetailsService {
        ...
        private final AdminClient userClient;
        /**
        * 事件发布器
        */
        ...
        public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
            ...
            if (Objects.isNull(userInfo)) {
                //发布登录失败的事件
                publisher.publishEvent(new UserLoginFailedEvent(username));
                throw new UsernameNotFoundException("用户名或者密码错误");
            }

    //        //角色和权限标识合并
            List<String> userPermission = Lists.newArrayList();
            if (CollectionUtils.isNotEmpty(userInfo.getRoleList())) {
                List<String> roleCollection = userInfo.getRoleList().stream().map(SysRole::getRoleCode).collect(Collectors.toList());
                userPermission.addAll(roleCollection);
            }
            if (Objects.nonNull(userInfo.getPermissions())) {
                userPermission.addAll(Lists.newArrayList(userInfo.getPermissions()));
            }
            String permissionStr = StringUtils.join(userPermission, ",");
            return new User(userInfo.getSysUser().getUsername(), userInfo.getSysUser().getPassword(), AuthorityUtils.commaSeparatedStringToAuthorityList(permissionStr));
        }
    }
    ~~~
* 重新运行，此时如果在1分钟内有3次登录失败，那么将会抛出账户锁定的异常。当然这段逻辑可以根据实际的业务需求来调整，如更新数据库对应的账户信息为锁定